...
impl SwapState for ConstantProductSwapV1 {
    ...
    fn check_pool_fee_info(&self, _: &AccountInfo) -> Result<(), ProgramError> {
        Ok(())
    }
...
impl Pack for ConstantProductSwapV1 {
    ...
    fn pack_into_slice(&self, output: &mut [u8]) {
        ...
        let (is_initialized, bump_seed, token_program_id, token_a, token_b, pool_mint, token_a_mint, token_b_mint, pool_fee_account, fees, swap_curve) =
...
mod tests {
    ...
    const TEST_CURVE: OffsetCurve = OffsetCurve { token_b_offset: TEST_TOKEN_B_OFFSET };
