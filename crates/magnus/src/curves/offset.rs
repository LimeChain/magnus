...
use crate::{
    curves::{
        calculator::{CurveCalculator, DynPack, RoundDirection, SwapWithoutFeesResult, TradeDirection},
        constant_product::{normalized_value, swap, withdraw_single_token_type_exact_out},
    },
    error::SwapError,
};
...
impl CurveCalculator for OffsetCurve {
    ...
    fn withdraw_single_token_type_exact_out(
        ...
        let token_b_offset = self.token_b_offset as u128;
        withdraw_single_token_type_exact_out(source_amount, swap_token_a_amount, swap_token_b_amount.checked_add(token_b_offset)?, pool_supply, trade_direction, round_direction)
    }

    fn validate(&self) -> Result<(), SwapError> {
        if self.token_b_offset == 0 { Err(SwapError::InvalidCurve) } else { Ok(()) }
    }
    ...
    fn normalized_value(&self, swap_token_a_amount: u128, swap_token_b_amount: u128) -> Option<PreciseNumber> {
        let token_b_offset = self.token_b_offset as u128;
        normalized_value(swap_token_a_amount, swap_token_b_amount.checked_add(token_b_offset)?)
    }
}
...
impl Pack for OffsetCurve {
    ...
    fn unpack_from_slice(input: &[u8]) -> Result<OffsetCurve, ProgramError> {
        ...
        Ok(Self { token_b_offset: u64::from_le_bytes(*token_b_offset) })
    }
}
...
mod tests {
    ...
    fn swap_offset() {
        ...
        let bad_result = curve.swap_without_fees(source_amount, swap_source_amount, swap_destination_amount, TradeDirection::BtoA);
