...
pub trait DexProcessor {
    ...
    fn after_invoke(&self, _account_infos: &[AccountInfo], _hop: usize, _owner_seeds: Option<&[&[&[u8]]]>, _before_sa_authority_lamports: u64) -> Result<u64> {
        Ok(0)
    }
}
...
pub fn before_check(
    ...
    if hop_accounts.from_account != ZERO_ADDRESS {
        require_keys_eq!(swap_source_token, hop_accounts.from_account, ErrorCode::InvalidHopAccounts);
        require_keys_eq!(swap_destination_token, hop_accounts.to_account, ErrorCode::InvalidHopAccounts);
    }
    if hop_accounts.last_to_account != ZERO_ADDRESS {
        require_keys_eq!(swap_source_token, hop_accounts.last_to_account, ErrorCode::InvalidHopFromAccount);
    }

    // check swap authority
    require!(swap_authority_pubkey.key() == swap_source_token_account.owner, ErrorCode::InvalidSwapAuthority);
    if !proxy_swap && hop == 0 {
        if owner_seeds.is_none() {
            require!(swap_authority_pubkey.is_signer, ErrorCode::SwapAuthorityIsNotSigner);
        }
    } else {
        require_keys_eq!(swap_authority_pubkey.key(), authority_pda::id(), ErrorCode::InvalidAuthorityPda);
    }
    Ok(())
}
...
fn post_swap_check<'info>(
    ...
    if swap_source_token.get_lamports() > 0 {
        ...
        let actual_amount_in = before_source_balance.checked_sub(after_source_balance).ok_or(ErrorCode::CalculationError)?;

        // min_amount_in = 90% of amount_in
        let min_amount_in =
            u64::try_from(u128::from(amount_in).checked_mul(ACTUAL_IN_LOWER_BOUND_NUM).and_then(|v| v.checked_div(ACTUAL_IN_LOWER_BOUND_DEN)).ok_or(ErrorCode::CalculationError)?)
                .map_err(|_| ErrorCode::CalculationError)?;
        if !(actual_amount_in <= amount_in && actual_amount_in >= min_amount_in) {
            msg!("InvalidActualAmountIn: actual_amount_in={}, amount_in={}", actual_amount_in, amount_in,);
    ...
    let actual_amount_out = after_destination_balance.checked_sub(before_destination_balance).ok_or(ErrorCode::CalculationError)?;
...
fn enforce_sa_token_allowlist(account_infos: &[AccountInfo], allowed_sa_token_accounts: &[Pubkey]) -> Result<()> {
            ...
            if let Ok(data) = ai.try_borrow_data() {
                if data.len() >= 165 {
                    if let Ok(ta) = SplTokenAccount::unpack(&data) {
                        if ta.owner == authority_pda::ID && !allowed_sa_token_accounts.contains(ai.key) {
