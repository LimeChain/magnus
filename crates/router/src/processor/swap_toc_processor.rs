...
impl SwapToCProcessor {
    ...
    fn calculate_from_fees<'info>(&self, amount_in: u64, commission_rate: u32, commission_direction: bool, platform_fee_rate: Option<u16>) -> Result<(u64, u64, bool)> {
        ...
        Ok((commission_amount, platform_fee_amount, commission_amount > 0 || platform_fee_amount > 0))
    }

    /// Calculate fees if commission is applied to to amount
    fn calculate_to_fees<'info>(&self, amount_out: u64, commission_rate: u32, commission_direction: bool, platform_fee_rate: Option<u16>) -> Result<(u64, u64, u64, bool)> {
        let mut commission_amount = 0;
        let mut platform_fee_amount = 0;
        ...
        if !commission_direction && commission_rate > 0 {
            ...
            actual_amount_out =
                actual_amount_out.checked_sub(commission_amount).ok_or(ErrorCode::CalculationError)?.checked_sub(platform_fee_amount).ok_or(ErrorCode::CalculationError)?;
        }

        Ok((commission_amount, platform_fee_amount, actual_amount_out, commission_amount > 0 || platform_fee_amount > 0))
    }
    ...
    fn transfer_from_fees_and_log<'info>(
        ...
        if is_charge_sol(commission_account, platform_fee_account, source_mint) {
            ...
            if commission_amount > 0 {
                ...
                let actual_fee_amount = transfer_sol_fee(payer, commission_account, commission_amount, None)?;
            ...
            if platform_fee_amount > 0 {
                ...
                let actual_fee_amount = transfer_sol_fee(payer, platform_fee_account, platform_fee_amount, None)?;
            ...
            if commission_amount > 0 {
                let commission_account = commission_account.as_ref().unwrap();
                transfer_token_fee(payer, &source_token_account.to_account_info(), source_mint, source_token_program, commission_account, commission_amount, None)?;
            ...
            if platform_fee_amount > 0 {
                let platform_fee_account = platform_fee_account.as_ref().unwrap();
                transfer_token_fee(payer, &source_token_account.to_account_info(), source_mint, source_token_program, platform_fee_account, platform_fee_amount, None)?;
    ...
    fn transfer_to_fees_and_log<'info>(
        ...
        if is_charge_sol(commission_account, platform_fee_account, destination_mint) {
            ...
            close_token_account(destination_token_account.to_account_info(), payer.to_account_info(), payer.to_account_info(), destination_token_program.to_account_info(), None)?;
            ...
            if commission_amount > 0 {
                let commission_account = commission_account.as_ref().unwrap();
                transfer_token_fee(payer, &destination_token_account.to_account_info(), destination_mint, destination_token_program, commission_account, commission_amount, None)?;
...
impl<'info> PlatformFeeV3Processor<'info> for SwapToCProcessor {
    ...
    fn before_swap(
        ...
        let (commission_amount, platform_fee_amount, is_charge_fee) = self.calculate_from_fees(amount_in, commission_rate, commission_direction, platform_fee_rate)?;
    ...
    fn after_swap(
        ...
        let (commission_amount, platform_fee_amount, actual_amount_out, is_charge_fee) =
            self.calculate_to_fees(amount_out, commission_rate, commission_direction, platform_fee_rate)?;
