...
impl Default for SwapCurve {
    fn default() -> Self {
        ...
        Self { curve_type, calculator: Arc::new(calculator) }
    }
}
    ...
    fn unpack_from_slice(input: &[u8]) -> Result<Self, ProgramError> {
        ...
        Ok(Self {
            ...
            calculator: match curve_type {
                CurveType::ConstantProduct => Arc::new(ConstantProductCurve::unpack_from_slice(calculator)?),
                CurveType::ConstantPrice => Arc::new(ConstantPriceCurve::unpack_from_slice(calculator)?),
                CurveType::Offset => Arc::new(OffsetCurve::unpack_from_slice(calculator)?),
                CurveType::Stable => unimplemented!("not yet implemented."),
            },
        })
    }
...
mod test {
    ...
    fn pack_swap_curve() {
        ...
        let swap_curve = SwapCurve { curve_type, calculator: Arc::new(curve) };
    ...
    fn constant_product_trade_fee() {
        ...
        let curve = ConstantProductCurve {};
        let swap_curve = SwapCurve { curve_type: CurveType::ConstantProduct, calculator: Arc::new(curve) };
        let result = swap_curve.swap(source_amount, swap_source_amount, swap_destination_amount, TradeDirection::AtoB, &fees).unwrap();
    ...
    fn one_sided_deposit_vs_swap(source_amount: u128, swap_source_amount: u128, swap_destination_amount: u128, pool_supply: u128, fees: Fees) -> (u128, u128) {
        let curve = ConstantProductCurve;
        let swap_curve = SwapCurve { curve_type: CurveType::ConstantProduct, calculator: Arc::new(curve) };
        // do the A to B swap
        let results = swap_curve.swap(source_amount, swap_source_amount, swap_destination_amount, TradeDirection::AtoB, &fees).unwrap();

        // deposit just A, get pool tokens
        let deposit_pool_tokens =
            swap_curve.deposit_single_token_type(results.source_amount_swapped, swap_source_amount, swap_destination_amount, pool_supply, TradeDirection::AtoB, &fees).unwrap();
    ...
    fn one_sided_equals_swap_with_fee_specific() {
        ...
        let fees = Fees { trade_fee_numerator: 25, trade_fee_denominator: 1_000, owner_trade_fee_numerator: 5, owner_trade_fee_denominator: 1_000, ..Fees::default() };
        let (withdraw_pool_tokens, deposit_pool_tokens) = one_sided_deposit_vs_swap(source_amount, swap_source_amount, swap_destination_amount, pool_supply, fees);
