...
impl Fees {
    ...
    pub fn fee_pct(&self) -> Option<Decimal> {
        let trade_fee_pct = if self.0.trade_fee_denominator > 0 {
            Decimal::from_u64(self.0.trade_fee_numerator)?.checked_div(Decimal::from_u64(self.0.trade_fee_denominator)?)?
        } else {
            Decimal::ZERO
        };

        let owner_trade_fee_pct = if self.0.owner_trade_fee_denominator > 0 {
            Decimal::from_u64(self.0.owner_trade_fee_numerator)?.checked_div(Decimal::from_u64(self.0.owner_trade_fee_denominator)?)?
        } else {
            Decimal::ZERO
        };
